"""
Do proposition coreference using argument coreference (gold argument coreference and gold predicate extraction) 
Author: Hitesh Golchha
Coref score achieved by best system:  [0.68, 0.81, 0.74, 0.74]
...
def evaluate_predicate_coref_using_everything_gold(test_graphs, arg_match_ratio, lexical_wt):
    """
    ...
    for graph in test_graphs:
        ...
        clusters = cluster_mentions(prop_mentions, score_prime, gold_arg_mentions, arg_match_ratio, lexical_wt)
...
# This score is not used. Was just for experimentation
...
def first_arg_match(prop_mention1, prop_mention2, gold_arg_mentions):
    ...
    :param prop_mention1: First proposition mention
    :param prop_mention2: Second proposition mention
    :param gold_arg_mentions: The gold argument mentions
...
def score_prime(prop, cluster, gold_arg_mentions, arg_match_ratio, lexical_wt):
    """
...
def cluster_mentions(mention_list, score, gold_arg_mentions, arg_match_ratio, lexical_wt):
    """
    ...
    for mention in mention_list:
        ...
        for cluster in clusters:
            if score(mention, cluster, gold_arg_mentions, arg_match_ratio, lexical_wt) > 0.5:
                cluster.add(mention)
                found_cluster = True
                break
...
def main():
    print "Experiments for Evaluating predicate coreference using gold argument coreference knowledge:"
    graphs = load_graphs_from_folder('../../data/baseline/test')

    ratios = [0.0, 0.25, 0.5, 0.75, 1.0]

    for lexical_wt in ratios:
        for arg_match_ratio in ratios:
            scores = evaluate_predicate_coref_using_everything_gold(graphs, arg_match_ratio, lexical_wt)
            print 'lexical_weight: {} argument_weight: {}: arg_match_ratio: {} score: {}'.format(lexical_wt, 1-lexical_wt, arg_match_ratio, scores)

def run_best_system():
    print "Running the proposition coreference system using argument coreference (gold)"
    graphs = load_graphs_from_folder('../../data/baseline/test')
    lexical_wt = 0.5
    arg_match_ratio = 0.75
    scores = evaluate_predicate_coref_using_everything_gold(graphs, arg_match_ratio, lexical_wt)
    print 'lexical_weight: {} argument_weight: {}: arg_match_ratio: {} score: {}'.format(lexical_wt, 1-lexical_wt, arg_match_ratio, scores)
