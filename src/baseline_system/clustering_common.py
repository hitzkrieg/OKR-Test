...
def cluster_mentions_changed(mention_list, score, clusters = [], merge_initially= False):
    """
    Cluster the predicate mentions in a greedy way: assign each predicate to the first
    cluster with similarity score > 0.5. If no such cluster exists, start a new one.
    ...
    :param merge_initially: Boolean value which indicates that initialised clusters needed to be merged before further clustering.
    This method was used for doing entity coreference from already initialized clusters incoming from corenlp (intra document).
    :return: clusters of mentions
    """
